...
static struct list_head myvivi_vb_local_queue; //本地队列
...
static void myvivi_buffer_queue(struct vb2_buffer *vb)
{
	...
	//将videobuf放入本地一个队列尾部，定时器处理函数就可以从本地队列取出此videobuf.
	list_add_tail(&vb>list, &myvivi_vb_local_queue->active);
}
...
{	//2.2，队列操作--队列初始化
	...
	//只有真正用到时才放到内核中去，这里会用到则加定时器:

	myvivi_timer.expires = jiff ies + 1;//超时时间
	add_timer(&myvivi_timer);
	
	memset(&myvivi_vb_vidqueue, 0, sizeof(myvivi_vb_vidqueue));
	return 0;
}
...
static int myvivi_mmap(struct file *filp, struct vm_area_struct *vm)
{
	struct video_device *vdev = video_devdata(filp);
	return vdev->fops->mmap(filp, vm);//调用我们自己的video_device结构中的mmap
}
...
// 1.3,注册
	...
	*/
	...
	myvivi_timer = myvivi_timer_function;//超时函数
